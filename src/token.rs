...
pub fn get_token_for_tenant(configuration: &super::ClientConfiguration, tenant: &String) -> Result<String> {
...
pub fn write_token_to_file(tenant: &String, token: &String) -> Result<(), Error> {
    ...
    trace!("Writing access token for tenant {} from file {}...", tenant, file_name);
    fs::write(file_name, token)
}

pub fn read_token_from_file(tenant: &String) -> Result<String, Error> {
    let file_name = resolve_file_name(&tenant);
    trace!("Reading access token for tenant {} to file {}...", tenant, file_name);
    fs::read_to_string(file_name)
}

pub fn invalidate_token(tenant: &String) -> Result<()> {
    let file_name = resolve_file_name(&tenant);
    trace!("Invalidating access token for tenant {} in file {}...", tenant, file_name);
...
fn request_new_token_from_provider(configuration: &super::ClientConfiguration, tenant: &String) -> Result<String> {
    ...
    match active_tenant {
        Some(active_tenant) => {
            ...
            let combined_credentials = [client_id.clone(), actual_client_secret.clone()].join(":").to_owned();
            ...
            let params = [("grant_type", "client_credentials"), ("scope", "tenantApp")];
        
            // Create the HTTP client instance
            //let client = reqwest::Client::new();
            let client = reqwest::blocking::Client::builder().timeout(Duration::from_secs(20)).build()?;
        
            let response = client.post(security_provider_url)
            ...
            match response {
                Ok(response) => {
                    ...
                    if status == StatusCode::OK {
                        ...
                        match response_text {
                            Ok(response_text) => {
                                let response: AuthenticationResponse = serde_yaml::from_str(&response_text).unwrap();
                                let token = response.access_token;
                                Ok(token)
                            },
                            Err(_) => {
                                Err(anyhow!(format!("Failed to obtain security token from the provider! Status: {:?}", status)))
                            }
                        }
                    } else {
                        Err(anyhow!(format!("Failed to obtain security token from the provider! Status: {:?}", status)))
                ...
                Err(_) => {
                    Err(anyhow!("Failed to obtain security token from the provider!"))
                }
            }
        },
        None => {
            Err(anyhow!("Unknown tenant {}", tenant))
        }
    }
}
